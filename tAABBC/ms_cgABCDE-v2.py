...
def create_representation():
    ...
    create_protein("ProteinB2", 175)
    create_protein("ProteinC", 100)
...
#        iD = r.add_type([rps[5]])
#        iE = r.add_type([rps[4]])
        n1 = r.add_composite([iA, iA, iB, iB, iC])
        n2 = r.add_composite([iA, iA, iB, iB], n1)
        n3 = r.add_composite([iA, iA], n2)
        n4 = r.add_composite([iA, iB], n2)
        n5 = r.add_composite([iB, iB, iC], n1)
        n6 = r.add_composite([iB, iB], n5)
        n7 = r.add_composite([iB, iC], n1)
#        n7 = r.add_composite([iB, iE], n5)
# Data from other sources: binary interacrions (x-linkis, affinity purificationP
#        n8 = r.add_composite([iB, iD], n1)
#        n9 = r.add_composite([iA, iC], n1)
#        n10= r.add_composite([iA, iE], n1)
#        n3 = r.add_composite([iA, iA], n2)
...
#    sA3=S(hierarchy=all, molecule="ProteinA3")
    sB1=S(hierarchy=all, molecule="ProteinB1")
    sB2=S(hierarchy=all, molecule="ProteinB2")
    sC=S(hierarchy=all, molecule="ProteinC")
    add_connectivity_restraint([sA1, sA2, sB1, sB2, sC])
...
f1 = open("out_scores_testAABBC-ver1.csv", "w")
...
for i in range(0, cs.get_number_of_configurations()):
    cs.load_configuration(i)
    w= IMP.display.PymolWriter("configuration.%d.pym"%i)
    for g in gs:
        w.add_geometry(g)
# Report solutions
